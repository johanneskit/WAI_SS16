...
public class ImageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			p_statement = connection.prepareStatement("SELECT Webcams FROM benutzer WHERE id = ?");
			p_statement.setInt(1, Integer.parseInt(user_id));
			...
			try (ResultSet resultSet = p_statement.executeQuery()) {
				if (resultSet.next()) {
					cam_id = resultSet.getString("cam_id");
				}
